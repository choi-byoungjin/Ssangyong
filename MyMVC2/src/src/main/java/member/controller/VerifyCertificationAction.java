...
public class VerifyCertificationAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String userCertificationCode = request.getParameter("userCertificationCode"); // 사용자가 보내준 인증코드
		String userid = request.getParameter("userid");		
		
		HttpSession session = request.getSession(); // 세션불러오기
		String certificationCode = (String)session.getAttribute("certificationCode"); // 세션에 저장된 인증코드 가져오기 // 랜덤하게 발생하는 인증코드		
	...
	//	!!! 중요 !!!
	//	!!! 세션에 저장된 인증코드 삭제하기 !!!
		session.removeAttribute("certificationCode"); // LogoutAction.java 참고
	}
