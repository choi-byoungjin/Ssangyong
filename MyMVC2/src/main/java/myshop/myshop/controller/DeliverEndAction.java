...
public class DeliverEndAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!isLogIn) {
			request.setAttribute("message", "배송완료를 조회하려면 먼저 로그인 부터 하세요!!");
		...
		//  super.setRedirect(false);
			...
			if(!"admin".equals(userid) ) {
				...
				/*
					s20220414-11/62 ',' s20220414-10/62 ',' s20220412-9/66 ',' s20220412-8/62 ',' s20220412-8/6 '
					s20220414-11 은 주문코드(전표)' 이고 /뒤에 붙은 60 은 제품번호이다.
					이것은 오라클에서 주문코드(전표)컬럼||'/'||제품번호컬럼으로 사용하기위한 데이터값을 만들려고 한다.
				 */
