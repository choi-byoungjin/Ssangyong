...
public class FileDownloadAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	=== 첨부되어지는 이미지 파일이 올라가는 절대경로 uploadFileDir ==> C:\NCS\workspace(jsp)\.metadata\.plugins\org.eclipse.wst.server.core\tmp1\wtpwebapps\MyMVC\images
			...
			Map<String, String> map = pdao.getPrdmanualFileName(pnum); // dao 에서 넘겨준 map
			...
			/*
				File.separator ==> 사용중인 운영체제가 Windows 이라면 File.separator 의 값은 "\" 이고
								   사용중인 운영체제가 매킨토시(MAC), Linux, Unix 이라면 File.separator 의 값은 "/" 이다.
	        ...
	        */
	        ...
	        int data = 0;
	        while( (data = finStream.read(arrb, 0, arrb.length)) != -1 ) { // 더 이상 읽을 것이 없으면 -1이 나온다.
