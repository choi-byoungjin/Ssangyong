...
public class LoginAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	System.out.println(">> 확인용 clientip => " + clientip);
			...
			// >> 확인용 clientip => 192.168.0.5 // 핸드폰으로 접속 // http://192.168.0.13:9090/MyMVC/
			...
			if( loginuser != null ) {
				...
				if(loginuser.getIdle() == 1 ) {
					String message = "로그인을 한지 1년이 지나서 휴면상태가 되었습니다. 관리자에게 문의 바랍니다.";
				...
				// 로그인을 한지 1년 이내인 경우는 아래의 명령을 수행한다.
				// 로그인 성공시 
				...
				if(loginuser.isRequirePwdChange() == true) { // dao에서 3개월이 지났으면 true 값을 준다.
