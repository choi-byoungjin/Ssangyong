...
public class MemberCtrl {
			...
			switch (s_Choice) {
				...
				case "2": // 로그인 or 로그아웃
					if("로그인".equals(login_logout)) {
						member = login(sc);	// 로그인 시도하기 // return 타입은 memberDTO
					}
					else {
						member = null; // 로그아웃하기
				...
				case "5": 
					if(member != null) {
						// 회원탈퇴
						int n = mdao.memberDelete( member.getUserseq() ); // return 값이 int
						
						if(n==1) {
							System.out.println(">>> 회원탈퇴가 성공했습니다. <<<\n");
							member = null; // null이 아니면 do-while문을 계속 돈다.
						}
					}
					else {
						System.out.println(">>> 메뉴에 없는 번호입니다. 다시 선택하세요! <<<\n");
	...
	private MemberDTO login(Scanner sc) {
		...
		member = mdao.login(paraMap); // DAO에서 호출되어진 결과값이 null이 아니면 로그인 
