        ...
        try {
			...
			*/ 
			...
			String sql = " select no, name, msg, to_char(writeday, 'yyyy-mm-dd hh24:mi:ss') AS writeday "
					   + " from jdbc_tbl_memo " // +는 문자열 결합 // 공백을 줘야 함
			...
			*/
			...
			StringBuilder sb = new StringBuilder(); // sb = string build
			
			while(rs.next()) {// 행이 없을때까지
				/*
			...
			*/
			// 또는
				int no = rs.getInt(1); 				// 1 은 select 해온 첫번째컬럼명이다.
				String name = rs.getString(2);		// 2 은 select 해온 두번째컬럼명이다.
				String msg = rs.getString(3);		// 3 은 select 해온 세번째컬럼명이다.
				String writeday = rs.getString(4);	// 4 은 select 해온 네번째컬럼명이다.
