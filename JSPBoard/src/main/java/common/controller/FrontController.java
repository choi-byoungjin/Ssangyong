...
@WebServlet(
		...
		urlPatterns = { "*.bd" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:\\Users\\choib\\git\\Ssangyong\\JSPBoard\\src\\main\\webapp\\WEB-INF\\Command.properties", description = "*.up 에 대한 클래스의 매핑파일")
		})
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {	// init은 환경설정
	/*
	...
	//	System.out.println("~~~ 확인용 => 서블릿 FrontController 의 init(ServletConfig config) 메소드가 실행됨.");
		...
		String props = config.getInitParameter("propertyConfig"); // propertyConfig를 넣으면 위의 value값이 나온다.
	//	System.out.println("~~~ 확인용 props => " + props);
		...
		try {
			...
			*/
			...
			//	System.out.println("### 확인용 value => " + pr.getProperty(key) );
				...
				*/
				...
				if(className != null) {
					...
					Class<?> cls = Class.forName(className); // className은 common.controller.MainController 등
			...
			System.out.println(">>> C:\\Users\\choib\\git\\Ssangyong\\JSPBoard\\src\\main\\webapp\\WEB-INF\\Command.properties 파일이 없습니다. <<<");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
		} catch (ClassNotFoundException e) { // properties에 없는 것을 쓰려고 할 때
	...
	//	System.out.println("~~~ 확인용 url => " + uri);
	...
	//	System.out.println("~~~ 확인용 key => " + key);		
		...
		AbstractController action = (AbstractController) cmdMap.get(key); // Map에 object 이므로 부모클래스로 받아온다
		...
		if(action == null) {
			...
			try {
				action.execute(request, response);	// 실제로 forward 해주는 서블릿
