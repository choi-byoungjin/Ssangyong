...
깃허브 overlaps 오류 발생시 git 폴더 내의 .project 삭제 후 
...
- 오버라이딩은 부모클래스로부터 상속받은 메소드를 내용물만 바꾸어서 재정의 하는 것
- 메소드 오버라이딩시 접근제한자는 부모클래스에서 주었던 접근제한자와 같거나 더 크게 허용하는 접근제한자만 가능하다.
- 오버로딩은 파라미터의 갯수, 순서, 타입만 달라지면 다른 메소드로 본다 리턴타입은 고려하지 않고 중복처리한다.
- 오버라이딩은 리턴타입, 파라미터, 메소드 이름 모두 동일해야 한다.
- 일반클래스는 객체를 생성할 수 있음, 추상클래스는 객체 생성할 수 없음
- 필드는 건드리지 못하게 private 해두고 메소드를 통해 접근하도록 메소드를 public을 지정한다.
- 다형성
...
링크 테스트 #0930f3dc1d47acffc29a8d71a2c84ceddb89f05b
#ca25bd1
...
HTML 문서 속에 Java 코드가 들어가 있는 것

서블릿(Servlet)
서블릿 클래스로 부터 만들어진 객체를 말한다.
웹컨테이너는 서블릿 클래스를 가지고 객체를 만든 다음에, 초기화해서 웹서비스를 할 수 있는 상태로 만든다.
