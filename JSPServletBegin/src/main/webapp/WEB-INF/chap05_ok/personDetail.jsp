...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("span#colorbox").css({'background-color':'${requestScope.psdto.color}'}); /* 서블릿을 통해 color를 불러온다 */
	});
...
<body class="py-3"> <%-- py는 y축 px는 x축 padding --%>

	<div class="container">
		<div class="card">
			<div class="card-header">	<%-- .card-header 에는 카드 머리말이 들어갑니다. --%>
				<p class="h2 text-center">
					<span class="text-primary"> ${requestScope.psdto.name}</span>&nbsp;<small>님의 개인성향 결과</small> <%-- small 태그는 상대적으로 작게 만들어준다. --%>
				</p>
			</div>
			
			<div class="card-body">	<%-- .card-body 에는 카드 내용이 들어갑니다. --%>
				<div class="row mx-4 my-3 border border-top-0 border-left-0 border-right-0 "> <%-- my는 y축 px는 m축 margin --%>
				...
				<div class="row mx-4 my-3 border border-top-0 border-left-0 border-right-0 ">
					...
					<div class="col-md-9 py-2"><span class="h5">${requestScope.psdto.strFoodImageFileName}</span></div> <%-- dto에서 이미지파일명 구현 --%>
				</div>
				
				<div class="row mx-4 my-3 border border-top-0 border-left-0 border-right-0 ">
					<div class="col-md-3 py-2">음식사진</div>
					<div class="col-md-9 py-2">
						<c:if test="${not empty requestScope.psdto.strFoodImageFileName}">
							<c:forTokens var="imgFileName" items="${requestScope.psdto.strFoodImageFileName}" delims=",">
								<img src="<%= ctxPath%>/chap05_images/${imgFileName}" width="130" class="img-fluid rounded mr-1" /> <%-- height 안줘도 자동으로 보정됨 --%> <%-- img-fluid는 반응형 이미지 --%>
							</c:forTokens>
						</c:if>
						
						<c:if test="${empty requestScope.psdto.strFoodImageFileName}">
			...
			<div class="card-footer">	<%-- .card-footer 에는 카드 내용이 들어갑니다. --%>
				<div class="row" style="position: relative; top:10px;">
					...
					<div class="col-md-3">
						<p class="text-center">
							<button type="button" class="btn btn-success" onclick="javascript:location.href='personSelectAll.do'">목록보기-2</button> <%-- 암기필요 --%>
						</p>
					</div>
					<div class="col-md-3">
						<p class="text-center">
	...
	<%-- POST 방식을 통해 회원을 삭제하기 위하여 화면에는 보이진 않는 form 태그를 만든다. --%>
	<form name="delFrm"> <%-- form 태그는 전송이 목적이다. --%>
