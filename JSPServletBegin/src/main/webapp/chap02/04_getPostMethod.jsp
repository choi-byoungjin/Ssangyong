...
<body>
    ...
    <%= ctxPath%> 이  /JSPServletBegin 이다.
  	...
  	<%-- 
		...
		http://localhost:9090/JSPServletBegin/04_getPostMethod.do 으로 데이터를 전송시킨다.
		method 를 생략하면 method="get" 으로 된다.
		   
		POST방식은  웹브라우저 주소창에  http://URL주소만나옴 으로 나타내주는 것이며 ? 가 없다.
		POST방식은 전송할 데이터를 웹브라우저 주소창 아닌 본문에 넣어서 보낸다.
	...
	<%-- 
		submit 을 하면 <%= ctxPath%>/04_getPostMethod.do 로 보내어서 처리를 요청한다.
		/JSPServletBegin/04_getPostMethod.do 은 누가 처리를 해주는지는 배치서술자인 web.xml 에 기술되어져 있어야 한다.
	    web.xml 에 가보면 <servlet-mapping>에 URL 패턴으로 /04_getPostMethod.do 가 기술되어져 있고,
