<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%> <!-- 언어는 자바, 내용물은 html, 한글을 지원함 -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<%
	...
	//	  스크립틀릿(scriptlet)이 자바코드가 들어오는 부분이다.
...
<!-- HTML 주석문 -->
<%-- JSP 주석문 --%> <%-- 웹 소스에 보이지 않는다 --%>

<%-- 
	<% 로 시작하여 %> 로 끝나는 것을 "스크립틀릿(scriptlet)" 이라고 부른다. 
	"스크립틀릿(scriptlet)"에 자바 코딩이 들어가는 것이다.
	그런데 "스크립틀릿(scriptlet)"에 코딩된 부분은 웹브라우저에서 소스보기를 하더라도 안 보여지므로 보안성이 좋다.
	...
	C:\NCS\workspace(jsp)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\work\Catalina\localhost\JSPServletBegin\org\apache\jsp\chap01\_01oneToTenSum_jsp.class 로 저장된다. 

	간혹 01oneToTenSum.jsp 라는 파일을 수정을 했지만, 수정한대로 안 먹을 경우가 혹가다가 있다.
	이럴때 해결방법은 탐색기에서
