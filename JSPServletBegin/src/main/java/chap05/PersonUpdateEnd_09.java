		...
		if("POST".equalsIgnoreCase(method)) {
			...
			try {
				...
				if(n==1) {
					...
					response.sendRedirect("personDetail.do?seq="+seq); // 이미 있는 url페이지로 이동하는 것
					return; // 페이지를 띄우고 끝내야한다 RequestDispatcher를 하면 안됨
				}
				else { // 업데이트가 정상적으로 안됐을 때
					// 즉, URL경로로 페이지의 이동을 시켜주어야 한다.
				...
				e.printStackTrace();
				path= "/WEB-INF/chap05_ok/personRegister_fail.jsp"; // 원래 각각 만들어야 함
			}
		}
		else {
			// "GET" 방식으로 넘어온 것이라면
